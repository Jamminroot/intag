...
namespace intag
{
	public static class IniUtils
	{
		...
		{
			var desktopIniFilepath = Path.Combine(folder, "Desktop.ini");
			if (!File.Exists(desktopIniFilepath)) return Constants.EmptyDesktopIniTemplate + propertyValue;
			...
			if (propertyIndex >= 0)
			{
				lines[propertyIndex] = Constants.MagicPropertyTemplate + propertyValue;
				return string.Join("\n", lines) + Constants.MagicSectionTemplate + propertyValue;
			}

			// prop not found
			lines.Insert(indexOfSection + 1, Constants.MagicPropertyTemplate + propertyValue);
			return string.Join("\n", lines);
		}
	}
}